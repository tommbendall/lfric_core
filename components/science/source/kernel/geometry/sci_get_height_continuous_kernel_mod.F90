...
!>
module sci_get_height_continuous_kernel_mod
  ...
  !> Psy layer.
  !>
  type, public, extends(kernel_type) :: get_height_continuous_kernel_type
    private
  ...
  contains
    procedure, nopass :: get_height_continuous_code
  end type

  !---------------------------------------------------------------------------
  ! Contained functions/subroutines
  !---------------------------------------------------------------------------
  public :: get_height_continuous_code
...
!! @param[in] rmultplicility  Reciprocal of DoF multiplicity
...
subroutine get_height_continuous_code(nlayers,                         &
                                      height,                          &
                                      chi_1, chi_2, chi_3,             &
                                      rmultiplicity,                   &
                                      planet_radius,                   &
  ...
  real(kind=r_def),    dimension(undf_x),      intent(in) :: rmultiplicity
  ...
  if ( (geometry == geometry_spherical) .and. &
    ...
    do k = 0, nlayers-1
      do df_x = 1, ndf_x
        ...
        height( map_x(df_x) + k ) = height( map_x(df_x) + k )                  &
            + rmultiplicity( map_x(df_x) + k ) * coord_radius - planet_radius
    ...
    do k = 0, nlayers-1
      do df_x = 1, ndf_x
        ...
        height( map_x(df_x) + k ) = height( map_x(df_x) + k )                  &
            + rmultiplicity( map_x(df_x) + k ) * height_at_dof

      end do
    end do
  end if

end subroutine get_height_continuous_code

end module sci_get_height_continuous_kernel_mod
