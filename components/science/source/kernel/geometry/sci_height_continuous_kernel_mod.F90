...
!!        freedom for a continuous function space.
!> @details Uses the chi coordinate fields to determine the height of nodes
!!          above the Earth's mean radius, for continuous function spaces.
module sci_height_continuous_kernel_mod
  ...
  !! Psy layer.
  type, public, extends(kernel_type) :: height_continuous_kernel_type
    private
  ...
  contains
    procedure, nopass :: height_continuous_code
  end type

  !-----------------------------------------------------------------------------
  ! Contained functions/subroutines
  !-----------------------------------------------------------------------------
  public :: height_continuous_code
...
!> @param[in]     rmultiplicity  Reciprocal of DoF multiplicity for height field
...
subroutine height_continuous_code(                                             &
  ...
  real(kind=r_def),    intent(in)    :: rmultiplicity(undf_h)
  ...
  if (is_spherical_xyz) then
    ...
    do df_h = 1, ndf_h
      ...
      height(h_b_idx:h_t_idx) = (                                              &
          height(h_b_idx:h_t_idx)                                              &
          + rmultiplicity(h_b_idx:h_t_idx)*(coord_radius(:) - planet_radius)   &
      )
    end do
  ...
  ! -------------------------------------------------------------------------- !
    ...
    do df_h = 1, ndf_h
      ...
      do df_chi = 1, ndf_chi
        ...
        ! Increment height field at this DoF
        height(h_b_idx:h_t_idx) = (                                            &
            height(h_b_idx:h_t_idx) + rmultiplicity(h_b_idx:h_t_idx)           &
                * chi_3(chi_b_idx:chi_t_idx)*basis_val                         &
        )
      end do
    end do
  end if

end subroutine height_continuous_code
